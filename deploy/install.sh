#!/bin/bash
#
# Fast-Frame Installation Script
# Fast-Frame 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/meitianwang/fast-frame/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="meitianwang/fast-frame"
INSTALL_DIR="/opt/fast-frame"
SERVICE_NAME="fast-frame"
SERVICE_USER="fast-frame"
CONFIG_DIR="/etc/fast-frame"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="Fast-Frame 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 Fast-Frame 服务："
    ...
    ["not_installed"]="Fast-Frame 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 Fast-Frame。"
    ...
    ["uninstall_complete"]="Fast-Frame 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 Fast-Frame"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 Fast-Frame"
    ...
    ["server_config_desc"]="配置 Fast-Frame 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="Fast-Frame Installation Script"
    ...
    # Completion
    ["install_complete"]="Fast-Frame installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start Fast-Frame service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading Fast-Frame..."
    ...
    ["not_installed"]="Fast-Frame is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove Fast-Frame from your system."
    ...
    ["uninstall_complete"]="Fast-Frame has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install Fast-Frame"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove Fast-Frame"
    ...
    ["server_config_desc"]="Configure Fast-Frame server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/fast-frame" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/fast-frame" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="fast-frame_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/fast-frame" "$INSTALL_DIR/fast-frame"
    chmod +x "$INSTALL_DIR/fast-frame"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/fast-frame"
}
...
User=fast-frame
Group=fast-frame
WorkingDirectory=/opt/fast-frame
ExecStart=/opt/fast-frame/fast-frame
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=fast-frame
...
start_service() {
    ...
    if systemctl start fast-frame; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u fast-frame -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable fast-frame 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status fast-frame"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u fast-frame -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart fast-frame"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop fast-frame"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if Fast-Frame is installed
    if [ ! -f "$INSTALL_DIR/fast-frame" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/fast-frame" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet fast-frame; then
        print_info "$(msg 'stopping_service')"
        systemctl stop fast-frame
    fi

    # Backup current binary
    cp "$INSTALL_DIR/fast-frame" "$INSTALL_DIR/fast-frame.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/fast-frame.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/fast-frame"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start fast-frame

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: Fast-Frame must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/fast-frame" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="fast-frame.backup.${current_version}"
        else
            backup_name="fast-frame.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/fast-frame" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop fast-frame 2>/dev/null || true
    systemctl disable fast-frame 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/fast-frame.service
    systemctl daemon-reload
